...
private Item getChipItemFromId(byte chipId) {
    return switch (chipId) {
        case PCReference.ChipIds.WHITE -> ModItems.WHITE_POKER_CHIP;
        case PCReference.ChipIds.RED -> ModItems.RED_POKER_CHIP;
        case PCReference.ChipIds.GREEN -> ModItems.GREEN_POKER_CHIP;
        case PCReference.ChipIds.BLUE -> ModItems.BLUE_POKER_CHIP;
        case PCReference.ChipIds.BLACK -> ModItems.BLACK_POKER_CHIP;
        case PCReference.ChipIds.PURPLE -> ModItems.PURPLE_POKER_CHIP;
        case PCReference.ChipIds.YELLOW -> ModItems.YELLOW_POKER_CHIP;
        case PCReference.ChipIds.PINK -> ModItems.PINK_POKER_CHIP;
        case PCReference.ChipIds.ORANGE -> ModItems.ORANGE_POKER_CHIP;
    ...
    public boolean damage(DamageSource source, float amount) {
        if (source.getAttacker() instanceof PlayerEntity player) {
            if (!this.getWorld().isClient) {
                if (player.isSneaking()) {
                    // Sneak attack: Take entire stack
                    takeEntireStack(player);
                } else {
                    // Attack: Take one chip from top
                    takeOneChip(player);
                }
            }
