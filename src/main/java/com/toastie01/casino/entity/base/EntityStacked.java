package com.toastie01.casino.entity.base;
...
/**
 * Abstract base class for entities that can be stacked (cards, chips, etc.).
 * Provides common functionality for managing stacks of items with visual stacking effects.
 */
public abstract class EntityStacked extends Entity {

    /** Maximum number of items that can be stacked in one entity */
    public static final byte MAX_STACK_SIZE = PCReference.STANDARD_DECK_SIZE;

    /** Data tracker for the stack contents */
    ...
    public EntityStacked(EntityType<? extends EntityStacked> type, World world) {
        ...
        this.setBoundingBox(new Box(
            -PCReference.ENTITY_BOX_SIZE, 0.0, -PCReference.ENTITY_BOX_SIZE, 
            PCReference.ENTITY_BOX_SIZE, PCReference.ENTITY_BOX_HEIGHT, PCReference.ENTITY_BOX_SIZE
        ));
    }
    ...
    public void createAndFillDeck() {
        Byte[] fullDeck = new Byte[PCReference.STANDARD_DECK_SIZE];
        for (byte i = 0; i < PCReference.STANDARD_DECK_SIZE; i++) {
            fullDeck[i] = i;
        }
    ...
    public void tick() {
        ...
        if (this.getWorld().isClient) {
            ...
            if (!this.isOnGround()) {
                this.setVelocity(this.getVelocity().add(0.0D, PCReference.REDUCED_GRAVITY, 0.0D)); // Reduced gravity
            } else {
                // Dampen movement when on ground to reduce bouncing
                Vec3d velocity = this.getVelocity();
                this.setVelocity(velocity.multiply(PCReference.FRICTION_MULTIPLIER, PCReference.FRICTION_MULTIPLIER, PCReference.FRICTION_MULTIPLIER)); // Add friction
            }
        }
